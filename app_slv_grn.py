...
CONFIG = {
    'pdf': {
        ...
        'llama_agent': 'SL Veggies GRN',            # <-- must match the agent name in LlamaCloud
        'drive_folder_id': '1QCBhTn2kohkc1fvZWtFGqJ5R3UeZ-3JX',   # <-- folder containing SL Veggies PDFs
        'spreadsheet_id': '1DNowuKF1gk0AVu2Ytt1wKZqCtzllhPIZTAwk5p7aCNQ',     # <-- target Google Sheet
        'sheet_range': 'slveggiesgrn',                # tab name for extracted rows
        'days_back': 7,
        'max_files': 1000,
    ...
    'logs': {
        ...
        'sheet_name': 'workflow_logs_slveggies',
...
PREFERRED_HEADERS = [
    # bill_details (flattened)
    'bill_number', 'invoice_number', 'bill_date', 'po_number', 'po_date',
    # buyer_details (flattened, prefixed)
    'buyer_name', 'buyer_address', 'buyer_gstin', 'buyer_state',
    # line_items fields
    'serial_number', 'item_name', 'fsn_number',
    'indent_quantity', 'received_quantity', 'return_quantity', 'grn_quantity',
    'price_per_unit', 'tax_rate_percentage', 'amount',
    # metadata
...
class SLVeggiesAutomation:
    ...
    def validate_extraction_quality(self, extracted_data: Dict, file_name: str) -> Dict:
        ...
        (buyer_details / bill_details / line_items).
        """
        issues = []
        ...
        required_fields = ['item_name', 'grn_quantity']
        optional_fields = ['serial_number', 'fsn_number', 'indent_quantity',
        ...
        # Check 3: Header (bill/buyer) fields
        bill = extracted_data.get('bill_details') or {}
        buyer = extracted_data.get('buyer_details') or {}
        missing_bill = [f for f in ['bill_number', 'invoice_number', 'bill_date', 'po_number', 'po_date'] if not bill.get(f)]
        missing_buyer = [f for f in ['name', 'gstin'] if not buyer.get(f)]
        if missing_bill:
            issues.append(f"Missing bill_details fields: {', '.join(missing_bill)}")
        if missing_buyer:
            issues.append(f"Missing buyer_details fields: {', '.join(missing_buyer)}")
    ...
    def process_extracted_data(self, extracted_data: Dict, file_info: Dict) -> List[Dict]:
        ...
        Flatten the SL Veggies extraction JSON:
          - one output row per line_items entry
          - bill_details fields repeated on every row
          - buyer_details fields repeated on every row (prefixed buyer_)
        ...
        items = extracted_data.get("line_items") or []
        if not items:
            self.log(f"No 'line_items' key found in {file_info['name']}", "WARNING")
            return rows
        ...
        header_data = {
            ...
            "po_date": bill.get("po_date", ""),
            # buyer_details (prefixed to avoid ambiguity)
        ...
        for item in items:
            ...
            # line item fields (fsn_number can legitimately be null per schema)
            row["serial_number"] = item.get("serial_number", "")
            row["item_name"] = item.get("item_name", "")
            row["fsn_number"] = item.get("fsn_number") if item.get("fsn_number") is not None else ""
            row["indent_quantity"] = item.get("indent_quantity", "")
            row["received_quantity"] = item.get("received_quantity", "")
            row["return_quantity"] = item.get("return_quantity", "")
            row["grn_quantity"] = item.get("grn_quantity", "")
            row["price_per_unit"] = item.get("price_per_unit", "")
            row["tax_rate_percentage"] = item.get("tax_rate_percentage", "")
            ...
            # Note: 0 is a valid quantity, so only drop None/"" (unlike the
            # Hyperpure script which also dropped empty strings via `not in`)
